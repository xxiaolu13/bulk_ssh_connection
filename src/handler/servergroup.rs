...
pub async fn get_group_by_id(
    ...
    let group = get_group_by_id_db(&data.db_pool, group_id)
        .await
        .map_err(|e| {
...
pub async fn create_group(
    ...
    let group = create_group_db(&data.db_pool, group.try_into()?)
        .await
        .map_err(|e| {
...
pub async fn delete_group_by_id(
    ...
    let _ = get_group_by_id_db(&data.db_pool, group_id)
        .await
        .map_err(|e| {
    ...
    if server.len() > 0 {
        warn!(
            "group{} deleted but the group have servers {:?}",
            group_id, server
        )
    }
    // 如果group下有server，则会将server置为null
    let ans = delete_group_by_id_db(&data.db_pool, group_id)
        .await
        .map_err(|e| {
...
pub async fn update_group_by_id(
    data: web::Data<AppState>,
    group_id: web::Path<i32>,
    newgroup: web::Json<UpdateGroup>,
) -> Result<HttpResponse, actix_web::Error> {
    let group_id = group_id.into_inner();
    let group = update_group_by_id_db(&data.db_pool, group_id, newgroup.try_into()?)
        .await
        .map_err(|e| {
