...
use connect_ok::handler::cron_job::{
    create_cronjob, delete_cronjob_by_id, get_all_cronjobs, get_cronjob_by_id, update_cronjob,
};
use connect_ok::handler::cron_log::{get_cron_log_by_server_ip, *};
use connect_ok::handler::notfound::not_found_handler;
use connect_ok::handler::server::{update_server_by_id, *};
        ...
        App::new()
            ...
            .service(
                web::scope("/server")
                    ...
                    .route("/{id}", web::put().to(update_server_by_id)) // 更新单个server根据server的id
                    .route("/group/{id}", web::get().to(get_server_by_group_id)), // 根据group的id查找server
            )
            .service(
                web::scope("/cronlog")
                    //  .route("",web::post().to(create_cron_log))
                    .route("/{id}", web::get().to(get_cron_log_by_job_id)) // 根据job id查询日志
                    .route("/server/{ip}", web::get().to(get_cron_log_by_server_ip)), // 根据server ip查询日志
            )
            .service(
                web::scope("/ssh")
            ...
            .service(
                web::scope("/cronjob")
                    ...
                    .route("/{id}", web::delete().to(delete_cronjob_by_id)), // 删除cronjob根据id
            )
