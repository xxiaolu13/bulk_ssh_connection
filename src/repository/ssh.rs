...
use crate::domain::{
    cron_log::CreateCronLog,
    ssh_configuration::{Client, Message, Session},
};
use crate::repository::cron_log::create_cron_log_db;
use crate::utils::crypto::*;
use actix_web::error::ErrorInternalServerError;
use actix_web::error::{ErrorGatewayTimeout, ErrorRequestTimeout};
use bytes::Bytes;
...
// 全局SSH并发信号量
static SSH_SEMAPHORE: once_cell::sync::Lazy<Arc<Semaphore>> = once_cell::sync::Lazy::new(|| {
...
pub async fn single_server_ssh_back(
    job_id: Option<i32>,
    pool: &PgPool,
    msg: Message,
    command: String,
) -> Result<(u32, String), actix_web::Error> {
    let ip = msg.ipaddr.unwrap_or("".to_string());
    let ip_port = format!("{}:{}", ip, msg.port);
...
pub async fn batch_server_ssh_back(
    job_id: Option<i32>,
    pool: &PgPool,
    msg: Message,
    command: String,
) -> Result<tokio::sync::mpsc::Receiver<Result<Bytes, std::io::Error>>, actix_web::Error> {
    ...
    while let Some(fat_result) = tasks.join_next().await {
        let final_msg = match fat_result {
            Ok((server_label, r)) => {
                let task_final_msg = match r {
                    Ok((_code, output)) => {
                        ...
                        let back = serde_json::to_string(&ssh_result).unwrap_or_else(|_| {
                            format!(
                                r#"{{"server":"{}","error":"JSON serialization failed"}}"#,
                                server_label
                            )
                        });
            ...
            Err(e) => {
                let a = format!("batch execute ssh task error for join set {}", e);

                warn!("batch execute ssh task error for join set {}", e);
                a.to_string()
            }
        };
...
async fn ssh_execute(
    ...
    let mut connect: russh::client::Handle<Client> = match timeout(
        CONNECTION_TIMEOUT,
        russh::client::connect(config, ip_port.clone(), Client),
    )
    .await
    {
        Ok(Ok(handle)) => {
            log_and_record!(
                ...
                ip_port,
                "INFO",
                format!("Connection success to {}", ip_port)
            );
            handle
        }
        Ok(Err(e)) => {
    ...
    match timeout(
        AUTH_TIMEOUT,
        connect.authenticate_password(user.clone(), password),
    )
    .await
    {
        Ok(Ok(AuthResult::Success)) => {
            log_and_record!(
                ...
                ip_port,
                "INFO",
                format!("{} Authentication success", ip_port)
            );
    ...
    let mut ssh = Session { session: connect };
    ...
    let (code, output) = match timeout(COMMAND_TIMEOUT, ssh.call(command.as_str())).await {
        Ok(Ok((code, output))) => {
            log_and_record!(job_id, pool, ip_port, &code.to_string(), &output);
            (code, output)
        }
        Ok(Err(e)) => {
